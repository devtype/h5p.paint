...
| `canvas.brushDefaults.paletteColors` | List of 1–5 `{ "color": "#RRGGBB" }` items when `colorMode` is `palette` (add/remove/reorder in the editor; item groups use `isSubContent` so ColorSelector gets a hex string). Color pickers allow typing HEX via Spectrum `showInput`. The default brush color is always shown first to learners. |
