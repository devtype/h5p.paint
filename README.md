...
| `canvas.brushDefaults.paletteColors` | List of 1–5 `{ "color": "#RRGGBB" }` items when `colorMode` is `palette` (add/remove/reorder in the editor). Color pickers allow typing HEX via Spectrum `showInput`. The default brush color is always shown first to learners. |
